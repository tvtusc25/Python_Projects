...
class Analysis:
    ...
    def c_scatter(self, x_axis, y_axis, color_axis, title):
        x = self.data.select_data([x_axis]).ravel()
        y = self.data.select_data([y_axis]).ravel()
        z = self.data.select_data([color_axis]).ravel()

        color_map = palettable.colorbrewer.sequential.Blues_9

        fig, ax = plt.subplots()
        scatter = ax.scatter(x, y, c=z, s=75, cmap=color_map.mpl_colormap, edgecolors= 'black')
        
        ax.set_xlabel(x_axis)
        ax.set_ylabel(y_axis)
        ax.set_title(title)
        cbar = fig.colorbar(scatter)
        cbar.ax.set_ylabel(color_axis)
        return fig, ax
